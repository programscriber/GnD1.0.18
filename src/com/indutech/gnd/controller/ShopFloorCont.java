...
public class ShopFloorCont {
	...
	@RequestMapping(value="/rtoscanbyRSNtorto")
	public ModelAndView rtoReturnscanbyRSNtorto(ModelMap model, Principal principal){
		String username = null;
		try {
			username = principal.getName();
			model.addAttribute("username", username);
		...
		int size=getShopFloorService().getCountofRTo();
		ModelAndView view=new ModelAndView("rtoscanbyRSNtorto");
		view.addObject("rtoCount",size);
		return view;
	} 
	...
	@RequestMapping("/rtoscanbyAWBtorto")
	public @ResponseBody ModelAndView rtoscanbyAWBtoRTo(ModelMap model, Principal principal){
		String username=null;
		try {
			username = principal.getName();
			model.addAttribute("username", username);
		...
		int size=getShopFloorService().getCountofRTo();
		ModelAndView view=new ModelAndView("rtoscanbyAWBtorto");
		view.addObject("rtoCount",size);
		return view;
	}
	...
	public String wareHouseHome(ModelMap model, Principal principal) {
	...
	public String rtoHome(ModelMap model, Principal principal) {
	...
	@RequestMapping(value = "/carddeliverywarehouse")
	public String wareHouseCardDelivery(ModelMap model, Principal principal) {
		try {	
			String name = principal.getName();
			model.addAttribute("username", name);
		} catch(NullPointerException ne) {
	...
	@RequestMapping(value = "/carddeliverbyAWBwarehouse")
	public String warehousecarddeliverbyAWB(ModelMap model, Principal principal) {
		try {	
			String name = principal.getName();
			model.addAttribute("username", name);
		} catch(NullPointerException ne) {
		...
		return "warehousecarddeliverbyAWB";
...
//		logger.info(cardpin+""+awb+""+rsn);
	...
	@RequestMapping(value = "/cardredispatchwarehouse")
	public String warehouseCardRedispatch(ModelMap model, Principal principal) {
		try {
			String name = principal.getName();
			model.addAttribute("username", name);
		} catch(NullPointerException ne) {
		...
		return "warehousecardredispatch";
...
//		}
	...
	@RequestMapping(value = "/rtochangeRTOAWB")
	public ModelAndView rtochangeRTOAWBByBranch(ModelMap model, Principal principal) {
		ModelAndView view = null;
		try {
		...
		try {
			view = new ModelAndView("rtochangeRTOAWB");
			view.addObject("bank", DataInitialization.getInstance().getBankInfo());
		} catch(Exception e) {
			logger.error(e);
			e.printStackTrace();
	...
	@RequestMapping(value = "/rtogettxnbydate")
	public @ResponseBody ModelAndView rtoGetFilesByTxnDate(ModelMap model, Principal principal,
			@RequestParam(value="fromdate") String fromDate,
			@RequestParam(value="todate") String toDate,
			@RequestParam(value="bankid") String bankId,
			@RequestParam(value="status") String status) {
		...
		try {
			...
			logger.info("status is : "+status);
			list = getShopFloorService().getRecordsList(fromDate, toDate,  status, bankId);
			mav = new ModelAndView("rtochangeRTOAWB");
			List<BankBO> bankBO = DataInitialization.getInstance().getBankInfo();
			BankBO bankObj = null;
			for(BankBO bank : bankBO) {
				if(bank.getBankId() == Long.parseLong(bankId)) {
