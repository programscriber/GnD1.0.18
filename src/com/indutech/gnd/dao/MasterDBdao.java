...
public class MasterDBdao {
	...
	public List productSearchDao(Map productDao) {
		...
		if (StringUtils.isNotEmpty(productDao.get("productName").toString())) {
			crit.add(Restrictions.like("productName",
	...
	@SuppressWarnings("rawtypes")
	public boolean getCheckForExistingProductAndSaveIt(Map productDao,Product product) {
		...
		if (StringUtils.isNotEmpty(productDao.get("productName").toString())) {
			crit.add(Restrictions.like("productName",
					productDao.get("productName").toString()));
			System.out.println("productName" + productDao.get("productName").toString());
		}
		
		List resultlist = crit.list();
		if(resultlist.size()>0)
		{
		Product productNew = (Product) resultlist.get(0);
		productNew.setProductName(product.getProductName());
		productNew.setBin(product.getBin());
		productNew.setShortCode(product.getShortCode());
		productNew.setStatus(product.getStatus());
		productNew.setForthLineRequired(product.getForthLineRequired());
		productNew.setPhotoCard(product.getPhotoCard());
		getSessionFactory().getCurrentSession().update(productNew);
		addProduct = true;
		}
		return addProduct;
	}

/*test*/ /*public List<Product> getProductToEdit(Long productId,String shortCode,String productName,String bin,Long status,Long fourthStatus,Long photoCard)*/
	public List<Product> getProductToEdit(Long productId)
		{
		List<Product> product =(List<Product>)getSessionFactory().getCurrentSession().createCriteria(Product.class).add(Restrictions.eq("productId", productId)).list();
		System.out.println(product.size());
		return product;
		}

/*test*/		public List<MasterDcms> getMasterDcms(Long dcmsId)
		{
			List<MasterDcms> list = (List<MasterDcms>)getSessionFactory().getCurrentSession().createCriteria(MasterDcms.class).add(Restrictions.eq("dcmsId", dcmsId)).list();
			return list;
		}
...
@Transactional
public List<Product> getProductDAO(Long productId)
{
	System.out.println(productId);
	List<Product> list = (List<Product>) getSessionFactory().getCurrentSession().createCriteria(Product.class).add(Restrictions.eq("productId", productId)).list();
	System.out.println(list.size());
	return list;
}

@Transactional
public int deleteProductDAO(Long productId)
{
	Query q = getSessionFactory().getCurrentSession().createSQLQuery("delete from master_product_t where product_id="+productId);
...
@Transactional
public int deleteBranchDAO(Long branchId)
{
	Query q = getSessionFactory().getCurrentSession().createSQLQuery("delete from master_branch_t where id="+branchId);
	int row = q.executeUpdate();
	return row;
